...
public class DayColorsUtils {
    ...
    public static void setCurrentMonthDayColors(Calendar day, Calendar today, TextView dayLabel,
        ...
        if (today.equals(day)) {
            Log.d("getCurrentDay", "equals: "+today.get(Calendar.MONTH)+" : "+today.get(Calendar.DAY_OF_MONTH)
            +" Day : "+day.get(Calendar.MONTH)+" : "+day.get(Calendar.DAY_OF_MONTH));
            setTodayColors(dayLabel, calendarProperties);
        } else if (EventDayUtils.isEventDayWithLabelColor(day, calendarProperties)) {
            Log.d("getCurrentDay", "isEventDayWithLabelColor: "+today.get(Calendar.MONTH)+" : "+today.get(Calendar.DAY_OF_MONTH)
                    +" Day : "+day.get(Calendar.MONTH)+" : "+day.get(Calendar.DAY_OF_MONTH));
            setEventDayColors(day, dayLabel, calendarProperties);
        } else if (calendarProperties.getHighlightedDays().contains(day)) {
            Log.d("getCurrentDay", "getHighlightedDays: "+today.get(Calendar.MONTH)+" : "+today.get(Calendar.DAY_OF_MONTH)
                    +" Day : "+day.get(Calendar.MONTH)+" : "+day.get(Calendar.DAY_OF_MONTH));
